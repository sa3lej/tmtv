...
{
	fprintf(stderr, "usage: tmtv-server [-A] [-b ip] [-h hostname] [-k keys_dir] [-L max_lifetime] [-m max_sessions] [-p listen_port] [-q ssh_port_advertized] [-T idle_timeout] [-V] [-w web_port] [-z sse_port] [-x] [-v]\n");
}
...
int main(int argc, char **argv, char **envp)
{
	...
	while ((opt = getopt(argc, argv, "Ab:h:k:L:m:p:q:T:Vw:z:xv")) != -1) {
		switch (opt) {
		case 'V':
		...
		case 'L':
			...
			break;
		case 'm':
			tmate_settings->max_sessions = atoi(optarg);
			if (tmate_settings->max_sessions < 1)
				tmate_settings->max_sessions = 1;
			break;
		case 'p':
	...
	if (tmate_settings->max_sessions == 0)
		tmate_settings->max_sessions = TMATE_DEFAULT_MAX_SESSIONS;

	tmate_info("tmtv-server %s starting: ssh_port=%d sse_port=%d "
		   "bind=%s proxy_protocol=%s authorized_keys_only=%s "
		   "idle_timeout=%ds max_lifetime=%ds max_sessions=%d",
		   TMTV_VERSION,
